...
function initializeDeclaredSchemes(buildConfigProtocols?: CustomScheme[]) {
	if (schemasInitialized) {
		return;
	}
	schemasInitialized = true;

	const configuredSchemes = Array.isArray(buildConfigProtocols) ? buildConfigProtocols : [];
	for (const entry of configuredSchemes) {
...
async function dispatchProtocolRequest(
	...
	const scheme = normalizeScheme(new URL(meta.url).protocol);
	...
	if (!handler) {
		if (native) {
			const body = textEncoder.encode("No protocol handler registered");
...
export const Protocol = {
	_initialize(protocols?: CustomScheme[]) {
		initializeDeclaredSchemes(protocols);
	},
	handle(scheme: string, handler: ProtocolHandler) {
		...
		if (!declaredSchemes.has(normalizedScheme)) {
			throw new Error(
				`Protocol scheme '${normalizedScheme}' must be declared in electrobun.config.ts before calling Protocol.handle()`,
			);
		}
