...
// before any WKWebView/CEF browser is created, so request-path reads can remain
// plain pointer loads without adding synchronization overhead to every request.
...
@implementation MyURLSchemeHandler
    ...
    startURLSchemeTask:(id<WKURLSchemeTask>)urlSchemeTask {
        ...
        if ([urlString hasPrefix:@"views://"]) {
            ...
            if (g_protocolRequestHandler) {
                ...
                g_protocolRequestHandler(requestId, self.webviewId, requestJsonCopy);
            } else {
    ...
    - (void)webView:(WKWebView *)webView stopURLSchemeTask:(id<WKURLSchemeTask>)urlSchemeTask {
        ...
        if (g_protocolRequestCancelledHandler) {
            g_protocolRequestCancelledHandler(requestId);
        }
  ...
  bool Open(CefRefPtr<CefRequest> request,
        ...
        if (urlStr.find("views://") == 0) {
            ...
            for (const auto& header : headerMap) {
                ...
                firstHeader = false;
                headersJson += "[\"" + escapeProtocolJSONString([NSString stringWithUTF8String:header.first.ToString().c_str()]) + "\",\"" + escapeProtocolJSONString([NSString stringWithUTF8String:header.second.ToString().c_str()]) + "\"]";
            }
            headersJson += "]";
            ...
            if (g_protocolRequestHandler) {
                char *requestJsonCopy = strdup(requestJson.c_str());
                g_protocolRequestHandler(requestId_, webviewId_, requestJsonCopy);
                hasResponseBlock = true;
            }
        } else {
